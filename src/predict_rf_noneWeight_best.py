...
def load_test_data() -> pd.DataFrame:
    ...
    if "ID" not in df_test.columns:
        raise ValueError("Il n'a pas de colonne ID, c'est pas normal !")
...
def prepare_test_features(df_test: pd.DataFrame, feature_names):
    ...
    if missing_cols:
        info(f"Il manque {len(missing_cols)} colonnes. On les rajoute avec des 0.")
...
def make_submission(df_test: pd.DataFrame, clf, X_test: pd.DataFrame):
    ...
    for i, cls in enumerate(y_pred):
        if cls not in (0, 1, 2):
            raise ValueError(f"Le modèle a prédit {cls}, cette classe n'est pas connue.")
        one_hot[i, cls] = 1
