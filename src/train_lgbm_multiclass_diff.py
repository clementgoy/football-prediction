...
def make_lgbm_multiclass(random_state=42):
    clf = lgb.LGBMClassifier(
        ...
        num_class=3,
        boosting_type="dart",      # gère mieux les classes difficiles (nuls)
        class_weight="balanced",   # important pour ne pas oublier les DRAW
        n_estimators=2000,
        learning_rate=0.03,
        num_leaves=63,
        max_depth=-1,
        ...
        min_child_samples=200,     # gros leaf → moins d’overfitting
        reg_alpha=0.1,
        reg_lambda=0.1,
        random_state=random_state,
        n_jobs=-1,
